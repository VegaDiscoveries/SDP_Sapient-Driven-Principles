<#
.SYNOPSIS
    Straight copy of sdp-solution-coordinator.ps1, with one targeted fix: where the original
    hard-errors on a null active_solution_task, this copy instead returns a graceful
    "no_shared_task" status (exit 0), since that condition is the expected, common case for the
    sdp-solution-phase-coordinator skill this script backs - not an error. Every other line is
    unchanged from the original; the shared-task dispatch logic below is kept as-is (dead code in
    this skill's actual usage, since it is only ever invoked when active_solution_task is null)
    rather than stripped out, per the "straight copy, targeted edit only" design decision.
...
.NOTES
    ...
    Writes:
      .sdp-solution-workflow/sessions/session-NNN.md  (one per dispatched target - shared-task
        path only; the phases-1-7 path writes its own session file directly in the LLM skill,
        not via this script)
      .sdp-solution-workflow/state.json               (last_session, cached_status
        corrections, updated; halt / cascade / all_verified transitions - shared-task path only)
      SDP-Solution.json                               (last_active_projects, updated,
        active_solution_task on an all_verified transition - shared-task path only)
    ...
    Stdout: single-line JSON result object - same shape as sdp-solution-coordinator.ps1's, plus
      the new "no_shared_task" status value.
    Exit codes: 0 = success, no_shared_task, halt, blocked, cascade, or all_verified (all are
      expected, non-error terminal states the calling skill branches on); 1 = error (script could
...
function Exit-Halt($stateObj, [string]$statePath, [string]$reason) {
    ...
    $r.halt_message = "Solution halted - $reason. Resolve this condition and run sdp-solution-phase-coordinator to resume."
...
if (-not $activeSolutionTask) {
    # [TARGETED FIX] Original (sdp-solution-coordinator.ps1) hard-errors here. A null
    # active_solution_task is the expected, common case for this skill - not an error - since
    # this script is only ever invoked when the phases-1-7 path applies. Return gracefully so the
    # calling skill can proceed to its own Step 2a.
    $r = New-ResultBase
    $r.status = "no_shared_task"
    Write-Result $r
    exit 0
}
...
if ($workflowStatus -eq "halted") {
    ...
    $r.status = "halt"
    $r.workflow_status = "halted"
    $r.halt_message = "Solution halted - $haltReason. Resolve this condition and run sdp-solution-phase-coordinator to resume."
    Write-Result $r
    exit 0
}
