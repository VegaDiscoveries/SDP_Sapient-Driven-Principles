...
- **Early** — immediately after **solution-level** Phase 1/3 are drafted for a concept cycle (via
  the now solution-scoped `sdp-solution-new-concept-intake`), before **solution-level** Phase 4 begins.
...
phase/cycle: invoke
`/sdp-create-banner icon=info row=0 row: Status | No tracked source document for [phase/cycle] — coverage check not applicable.`
Stop. (Covers phases that predate this convention or were never sourced from an
...
If any element has no resolution yet: list the still-unresolved elements as plain text first,
then invoke
`/sdp-create-banner icon=warning row=0 row: Status | Source coverage check: [source doc] — not yet certified. [N] element(s) still unresolved. See list above.`
Do not certify. This check is not complete until every extracted element has one of the three
resolutions above.
...
```json
"sdp_source_coverage": {
  "purpose": "Records that sdp-solution-source-coverage-check has certified this phase's tracked source document as fully accounted for downstream. Does not substitute for sdp-project-doc-review or gate review.",
  "completed": true,
