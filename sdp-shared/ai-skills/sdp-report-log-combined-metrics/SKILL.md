...
1. **Resolve which day's file to read.**
   - List available files via the PowerShell tool:
     ...
     `/sdp-create-banner icon=error row=0 row: Status | No combined-log files found under .sdp-solution-workflow/logging/combined-logs/. Run sdp-report-logs-combine for the target day first.`
     and stop — do not proceed to Step 2. This is the one halt condition
...
relative from `sdp-solution-docs/log-reports/combined-metrics/` back to the solution root:
...
event count, the source mix (e.g. "3827 hook-log / 19 loop-metrics / 1 workflow-log"), and
`concerningCount` (call it out explicitly if non-zero), e.g.
`icon=success row=0 row: Report | [report_path] written — period: [period]. [source mix]. concerningCount: [N].`
