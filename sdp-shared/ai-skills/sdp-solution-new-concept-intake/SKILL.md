...
- **Document-driven** — one or more files are sitting in the drop zone. Runs the full
  ...
  later `sdp-solution-source-coverage-check` passes.
- **Conversational** — the user describes a new concept directly, with no document to drop.
  ...
  document is tracked, since none exists — `sdp-solution-source-coverage-check` already handles "no tracked source"
...
5. **Document-driven mode only:** record the moved path as the tracked source reference in the
   ...
   `processed/` location) — this is what `sdp-solution-source-coverage-check` reads on later passes.
...
6. Invoke `/sdp-create-banner` with a `Registry` row reporting the new registry rows and file
   paths created for this set, e.g.
   `icon=success row=0 row: Registry | Added [N] rows to registry.md: [phase names] — files: [file paths].`
...
- **Document-driven mode:** invoke the same banner with the Step 6 coverage-check row appended
  ...
  `/sdp-create-banner icon=success row=0 row: Status | Mini concept cycle seeded for [set] as
  phases [list] in the solution's registry.md. Run COORDINATOR next to begin Phase 1 drafting.
  row: Coverage | once Phase 1 and Phase 3 are drafted for this cycle,
  sdp-solution-source-coverage-check must run against them before Phase 4 begins — this is a mandatory
...
Phase 3 are drafted for this cycle, `sdp-solution-source-coverage-check` **must** run against them before
...
`sdp-solution-source-coverage-check` will self-report "not applicable" if it's ever run against this
...
- In conversational mode, never write a `source_document` field or otherwise fabricate a tracked
  ...
  `sdp-solution-source-coverage-check`'s own "no tracked source" path expects.
...
- Never begin Phase 4 for a cycle before `sdp-solution-source-coverage-check` has run against its drafted
