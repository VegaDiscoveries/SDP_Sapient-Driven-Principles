---
name: sdp-project-doc-review
description: Use when a staged SDP document has not been certified by doc review, when a user invokes doc review on a design or spec document, or when COORDINATOR detects a staged doc with no sdp_doc_review.completed entry.
---
...
1. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-project-doc-review" -event "start"` via the PowerShell tool.
2. Use the Read tool to read `sdp-shared/ai-skills/sdp-project-doc-review/SKILL.md` from the project
   root — do this before anything else. If the Read fails, halt and invoke
   `/sdp-create-banner icon=error row=0 row: Status | sdp-shared/ai-skills/sdp-project-doc-review/SKILL.md not found — skill cannot execute.`
3. Execute every numbered step in that SKILL.md in order, completing all sub-steps of a step
   ...
   is done.
4. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-project-doc-review" -event "end"` via the PowerShell tool.
