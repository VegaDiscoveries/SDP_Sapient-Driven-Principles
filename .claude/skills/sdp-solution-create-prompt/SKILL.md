---
name: sdp-solution-create-prompt
description: Write sdp-solution-docs/00_solution_prompt.txt with the solution sentinel and five-section prompt structure on behalf of sdp-solution-coordinator. Invoked by sdp-solution-coordinator — not by users directly.
---
...
1. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-solution-create-prompt" -event "start"` via the PowerShell tool.
2. Use the Read tool to read `sdp-shared/ai-skills/sdp-solution-create-prompt/SKILL.md` from the
   project root — do this before anything else. If the Read fails, halt: invoke
   `/sdp-create-banner icon=error row=0` with
   `row: Error | sdp-shared/ai-skills/sdp-solution-create-prompt/SKILL.md not found — skill cannot execute.`
3. Execute every numbered step in that SKILL.md in order, completing all sub-steps of a step
   ...
   is done.
4. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-solution-create-prompt" -event "end"` via the PowerShell tool.
