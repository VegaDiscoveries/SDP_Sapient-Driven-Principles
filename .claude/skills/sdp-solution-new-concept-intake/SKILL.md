---
name: sdp-solution-new-concept-intake
...
1. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-solution-new-concept-intake" -event "start"` via the PowerShell tool.
2. Use the Read tool to read `sdp-shared/ai-skills/sdp-solution-new-concept-intake/SKILL.md` from the
   project root — do this before anything else. If the Read fails, halt and invoke
   `/sdp-create-banner icon=error row=0 row: Status | sdp-shared/ai-skills/sdp-solution-new-concept-intake/SKILL.md not found — skill cannot execute.`
3. Execute every numbered step in that SKILL.md in order, completing all sub-steps of a step
   ...
   is done.
4. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-solution-new-concept-intake" -event "end"` via the PowerShell tool.
