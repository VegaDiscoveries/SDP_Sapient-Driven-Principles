---
name: sdp-cancel-auto
description: Use when the SDP auto-loop is running and needs to be stopped — lists active cron jobs, identifies sdp-project-state-loop entries, and cancels them.
---
...
1. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-cancel-auto" -event "start"` via the PowerShell tool.
2. Use the Read tool to read `sdp-shared/ai-skills/sdp-cancel-auto/SKILL.md` from the project
   root — do this before anything else. If the Read fails, halt and invoke
   `/sdp-create-banner icon=error row=0 row: Error | sdp-shared/ai-skills/sdp-cancel-auto/SKILL.md not found — skill cannot execute.`
3. Execute every numbered step in that SKILL.md in order, completing all sub-steps of a step
   ...
   is done.
4. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-cancel-auto" -event "end"` via the PowerShell tool.
