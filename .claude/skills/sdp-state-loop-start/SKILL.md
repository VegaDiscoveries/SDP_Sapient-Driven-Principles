---
name: sdp-state-loop-start
description: Validates SDP-Solution.json last_active_projects is non-empty, then starts the recurring SDP state-loop, then runs COORDINATOR once to prime an accurate first-fire dispatch prompt.
---
...
1. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-state-loop-start" -event "start"` via the PowerShell tool.
2. Use the Read tool to read `sdp-shared/ai-skills/sdp-state-loop-start/SKILL.md` from the project
   root — do this before anything else. If the Read fails, halt and invoke
   `/sdp-create-banner icon=error row=0 row: Error | sdp-shared/ai-skills/sdp-state-loop-start/SKILL.md not found — skill cannot execute.`
3. Execute every numbered step in that SKILL.md in order, completing all sub-steps of a step
   ...
   is done.
4. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-state-loop-start" -event "end"` via the PowerShell tool.
