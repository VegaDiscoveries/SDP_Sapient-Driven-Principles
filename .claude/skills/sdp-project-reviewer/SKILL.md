---
name: sdp-project-reviewer
description: Run a REVIEWER session — independently verify a completed task against its acceptance criteria, record the evaluation, and update state.
---
...
1. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-project-reviewer" -event "start"` via the PowerShell tool.
2. Use the Read tool to read `sdp-shared/ai-skills/sdp-project-reviewer/SKILL.md` from the project
   root — do this before anything else. If the Read fails, halt by invoking:
   ```
   /sdp-create-banner icon=error row=0
   row: Error | sdp-shared/ai-skills/sdp-project-reviewer/SKILL.md not found — skill cannot execute.
   ```
3. Execute every numbered step in that SKILL.md in order, completing all sub-steps of a step
   ...
   is done.
4. Run `./sdp-shared/scripts/sdp-tone.ps1 -skillName "sdp-project-reviewer" -event "end"` via the PowerShell tool.
