...
2. Run via the PowerShell tool. `SDP-Config.json`'s `newTerminals` array holds one or more named
   ...
   ```
   ./sdp-shared/scripts/sdp-claude-new-terminal.ps1 -terminal "[optional id or name]" -promptOverride "[optional override]"
   ```
3. Parse the single-line JSON result and report to the user:
   - `status: "success"` — "Launched a new terminal (instance `[instanceId]`, profile
     `[selectedTerminalId]:[selectedTerminalName]`, PID `[pid]`, via `[launcher]`) in
     `[resolvedDirectory]`." Append ", with initial prompt `[resolvedPrompt]`" only when
     `resolvedPrompt` is non-empty. Append ", permission mode `[resolvedPermissionMode]`" only
     when `resolvedPermissionMode` is non-empty. Append " ([markedNotRunningCount] session(s)
     marked not running, [prunedCount] pruned)" only when `markedNotRunningCount` or
     `prunedCount` is greater than 0.
   - `status: "error"` — report the `error` field verbatim to the user. Do not retry
     automatically. This covers: an unresolved `-terminal` selector (no matching `id`/`name` in
     `newTerminals`), and an unrecognized `permissionMode` value on the selected profile — no
     terminal is launched in either case.
